...
## ⚡ [4.4.4] - 21 de Febrero 2026 - **Fix hidratación Header (logo)**

### 🐛 Hydration mismatch en `/es/faqs` y rutas con Header

**Problema**: Error "Hydration failed because the server rendered HTML didn't match the client" en páginas que usan el Header (p. ej. `/es/faqs`). El servidor renderizaba un placeholder (`div`) para el logo y el cliente el `<img>` de Next.js `Image`, generando distinto árbol HTML.

**Solución**: Sustituir el componente `next/image` del logo en el Header por un `<img>` nativo con el mismo `src`, `alt`, dimensiones y clases. Servidor y cliente generan el mismo markup y desaparece el mismatch.

#### Archivos modificados

- `src/components/layout/header.tsx`: logo con `<img>` + eliminado import de `next/image`

#### Documentación

- **README.md**: sección "Última actualización" 21 Feb 2026
- **docs/INDICE-DOCUMENTACION.md**: referencia al fix
