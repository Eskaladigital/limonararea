  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
      ...
      <div className="p-6">
        ...
        {/* Specs en flex wrap (coherente con /parcelas) */}
        ...
        {vehicle.parcel_equipment && vehicle.parcel_equipment.length > 0 && (
          <div className="mb-4">
            <ParcelEquipmentDisplay
              equipment={vehicle.parcel_equipment}
              variant="icons"
              maxVisible={6}
              showIsofixBadge={true}
              hasIsofix={vehicle.parcel_equipment?.some((item: any) => item?.slug === 'isofix' || item?.equipment?.slug === 'isofix')}
            />
          </div>
        )}
