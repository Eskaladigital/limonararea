...
const iconMap: Record<string, LucideIcon> = {
  ...
  CheckCircle,
  Baby,
  // Parcelas: equipment del schema (superficie, orientación, sombra, césped...)
  maximize: Maximize2,
  sun: Sun,
  umbrella: Umbrella,
  "tree-pine": TreePine,
...
const categoryColors: Record<string, { bg: string; text: string; icon: string }> = {
  ...
  parcelas: { bg: "bg-emerald-50", text: "text-emerald-700", icon: "text-emerald-500" },
...
export function legacyToEquipment(parcel: LegacyEquipmentProps): Equipment[] {
  const equipment: Equipment[] = [];
  
  if (parcel.has_kitchen) {
    equipment.push({ id: 'legacy-kitchen', name: 'Cocina', slug: 'cocina', icon: 'UtensilsCrossed', category: 'confort' });
  }
  if (parcel.has_bathroom) {
...
/**
 * Componente que muestra equipamiento de una parcela
 * Usa el nuevo sistema si hay equipment, si no usa legacy
 */
interface ParcelEquipmentDisplayProps {
  equipment?: Equipment[];
...
export function ParcelEquipmentDisplay({
  equipment,
  legacyData,
  variant = "badges",
  maxVisible,
