...
export default async function VehicleDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
<main className="min-h-screen bg-sand-lt overflow-x-hidden md:pt-0">
        <div className="bg-white border-b">
          <div className="container mx-auto px-4 py-4">
            <LocalizedLink href="/parcelas" className="inline-flex items-center gap-2 text-gray-600 hover:text-clay">
        ...
        <div className="container mx-auto px-4 py-4 md:py-8 max-w-7xl overflow-hidden">
          ...
          <div className="lg:hidden bg-white rounded-xl shadow-sm p-4 mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-clay">{formatPrice(parcel.base_price_per_day)}<span className="text-sm font-normal text-gray-500">{t("/día")}</span></p>
            </div>
            <LocalizedLink href={`/reservar?parcela=${parcel.slug}`} className="bg-clay text-white font-semibold py-2.5 px-5 rounded-lg hover:bg-clay-dk transition-colors text-sm">
              {t("Reservar")}
            </LocalizedLink>
          </div>
            ...
            <div className="lg:col-span-2 space-y-4 md:space-y-6">
              ...
              <div className="bg-white rounded-xl md:rounded-2xl shadow-sm p-4 md:p-6">
                <span className="px-2 md:px-3 py-1 bg-clay/10 text-clay rounded-full text-xs md:text-sm font-medium">
                  {parcel.category?.name || t("Parcela")}
                </span>
                <h1 className="text-xl md:text-3xl font-bold text-gray-900 mt-3 md:mt-4 mb-4 md:mb-6">{parcel.name}</h1>
              ...
              {/* Dimensiones */}
              {(parcel.length_m || parcel.width_m || parcel.height_m) && (
                <div className="bg-white rounded-xl md:rounded-2xl shadow-sm p-4 md:p-6">
            ...
            <div className="hidden lg:block space-y-6">
              <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
                <div className="text-center mb-6">
                  ...
                  <p className="text-4xl font-bold text-clay">{formatPrice(parcel.base_price_per_day)}<span className="text-lg font-normal text-gray-500">{t("/día")}</span></p>
                </div>
                <LocalizedLink href={`/reservar?parcela=${parcel.slug}`} className="block w-full bg-clay text-white text-center font-semibold py-3 px-4 rounded-lg hover:bg-clay-dk transition-colors mb-3">
                  {t("Reservar ahora")}
                </LocalizedLink>
                <LocalizedLink href="/tarifas" className="block w-full border-2 border-gray-200 text-gray-700 text-center font-semibold py-3 px-4 rounded-lg hover:bg-sand-lt transition-colors">
                  {t("Ver tarifas")}
                </LocalizedLink>
                <div className="mt-6 pt-6 border-t border-gray-100">
                  <h3 className="font-semibold text-gray-900 mb-3">{t("¿Tienes dudas?")}</h3>
                  <a href="tel:+34868364161" className="text-clay hover:underline">868 36 41 61</a>
                </div>
              </div>
            </div>
          </div>
