...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const destinatario = testEmail || process.env.COMPANY_EMAIL || 'reservas@limonar.com';

    // Enviar email de prueba
    const sendResult = await sendEmail({
      to: destinatario,
      subject: '🧪 Test de Email - Eco Area Limonar',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; padding: 40px; background-color: #f5f5f5;">
          <div style="max-width: 600px; margin: 0 auto; background: white; padding: 40px; border-radius: 8px;">
            <div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #f97316 0%, #ea580c 100%); border-radius: 8px;">
              <h1 style="color: white; margin: 0;">🚐 Eco Area Limonar</h1>
            </div>
            
            <div style="padding: 30px 0;">
              <h2 style="color: #111827;">✅ ¡El sistema de emails funciona!</h2>
              
              <p>Este es un email de prueba enviado desde el sistema de Eco Area Limonar.</p>
            ...
            <div style="text-align: center; padding: 20px; background-color: #f9fafb; border-radius: 8px;">
              <p style="margin: 0; color: #6b7280; font-size: 14px;">
                Eco Area Limonar - Alquiler de Campers<br>
                <a href="https://www.limonar.com" style="color: #f97316;">www.limonar.com</a>
              </p>
            </div>
          </div>
        </body>
