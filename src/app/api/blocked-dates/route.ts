...
const blockedDateSchema = z.object({
  parcel_id: z.string().uuid("ID de parcela inválido"),
...
export async function POST(request: Request) {
  try {
    ...
    const { parcel_id, start_date, end_date, reason, created_by } = validationResult.data;
    ...
    if (existingBookings && existingBookings.length > 0) {
      return NextResponse.json(
        {
          error: `Ya existen ${existingBookings.length} reserva(s) para esta parcela en las fechas seleccionadas`,
    ...
    if (existingBlocks && existingBlocks.length > 0) {
      return NextResponse.json(
        {
          error: `Ya existe un bloqueo para esta parcela en las fechas seleccionadas`,
        },
        { status: 409 }
      );
    }
