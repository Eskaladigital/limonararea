...
export async function PUT(
  ...
  try {
    ...
    const parcelId = updates.parcel_id || currentBlock.parcel_id;
    ...
    if (existingBookings && existingBookings.length > 0) {
      return NextResponse.json(
        {
          error: `Ya existen reservas para esta parcela en las fechas seleccionadas`,
    ...
    if (existingBlocks && existingBlocks.length > 0) {
      return NextResponse.json(
        {
          error: `Ya existe otro bloqueo para esta parcela en las fechas seleccionadas`,
        },
        { status: 409 }
      );
    }
