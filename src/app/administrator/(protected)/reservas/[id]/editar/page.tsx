...
export default function EditarReservaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Cargar parcelas ordenados por código interno (como en el calendario)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // VALIDACIÓN CRÍTICA: Verificar BLOQUEOS del parcela
      ...
      if (blockedError) {
        ...
        setMessage({ type: 'error', text: 'Error al verificar bloqueos del parcela' });
        setSaving(false);
        return;
      }
      ...
      if (blockedDates && blockedDates.length > 0) {
        ...
        setMessage({ 
          type: 'error', 
          text: `🚫 NO SE PUEDE GUARDAR — ${parcelName} tiene BLOQUEO activo:\n\n${blockInfo}\n\nSelecciona otras fechas o elige un parcela diferente.`
        });
        setSaving(false);
        return;
      }

      // VALIDACIÓN CRÍTICA: Verificar reservas conflictivas del parcela
      ...
      if (checkError) {
        ...
        setMessage({ type: 'error', text: 'Error al verificar disponibilidad del parcela' });
        setSaving(false);
        return;
      }
      ...
      if (conflictingBookings.length > 0) {
        ...
        setMessage({ 
          type: 'error', 
          text: `🚫 NO SE PUEDE GUARDAR — ${parcelName} ya tiene ${conflictingBookings.length} reserva(s) en esas fechas:\n\n${conflictInfo}\n\nSelecciona otras fechas o elige un parcela diferente.`
        });
        setSaving(false);
        return;
      }
  ...
  const handleSendEmail = async (emailType: 'booking_created' | 'first_payment' | 'second_payment') => {
    ...
    if (!confirm(`¿Enviar email de "${emailLabels[emailType]}" al cliente y a ${process.env.COMPANY_EMAIL || 'reservas@ejemplo.com'}?`)) {
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <Link 
            ...
            className="inline-flex items-center text-sm text-gray-600 hover:text-clay mb-2 transition-colors"
          >
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        ...
        <p className="text-xs text-blue-600 mt-2">
          El email se enviará al cliente ({formData.customer_email}) y a {process.env.COMPANY_EMAIL || 'reservas@ejemplo.com'}
        </p>
      </div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {customerId && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-3 py-1.5 text-sm bg-clay text-white rounded-lg hover:bg-clay-dk transition-colors"
                  >
                    Editar cliente
                  </button>
                )}
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-gray-200">
                  ...
                  <div className="flex justify-between text-lg font-bold pt-2 border-t border-gray-200">
                    <span>Total:</span>
                    <span className="text-clay">{formatPrice(formData.total_price)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <button
                type="submit"
                disabled={saving}
                className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 bg-clay text-white rounded-lg hover:bg-clay-dk transition-colors disabled:opacity-50 font-semibold"
              >
                {saving ? (
                  <>
