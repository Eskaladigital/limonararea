...
const CATEGORIES = [
  { value: "parcela", label: "Parcela", color: "bg-emerald-100 text-emerald-700" },
...
export default function CaracteristicasPage() {
  ...
  useEffect(() => {
    document.title = "Admin - Características | Eco Area Limonar";
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === "23505") {
        toast.error("Ya existe una característica con ese slug");
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Eliminar esta característica? Se quitará de todas las parcelas.")) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Gestiona las características de las parcelas: superficie, orientación, agua, césped, etc.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          className="flex items-center gap-2 bg-clay text-white px-4 py-2 rounded-lg hover:bg-clay-dk transition-colors"
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-clay focus:border-transparent"
                  placeholder="Ej: Agua, Césped, Orientación Norte"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-clay focus:border-transparent"
                  >
              ...
              <div className="md:col-span-2 flex flex-wrap gap-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 text-clay rounded focus:ring-clay"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 text-clay rounded focus:ring-clay"
                  />
                  <span className="text-sm text-gray-700">Estándar en todas las parcelas</span>
                </label>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-4 border-t">
              <button
                type="submit"
                disabled={saving}
                className="flex items-center gap-2 bg-clay text-white px-4 py-2 rounded-lg hover:bg-clay-dk transition-colors disabled:opacity-50"
              >
                {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
                {editingId ? "Guardar cambios" : "Crear característica"}
              </button>
              <button
                type="button"
                onClick={cancelForm}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="text-left py-3 px-4 text-sm font-semibold text-gray-600">Característica</th>
            ...
            {filteredEquipment.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-12 text-center text-gray-500">
                  No hay características en esta categoría
                </td>
              </tr>
            ) : (
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-2 text-gray-500 hover:text-blue-600 hover:bg-earth/10 rounded-lg transition-colors"
                          title="Editar"
                        >
      ...
      <div className="bg-blue-50 rounded-xl p-4 text-sm text-blue-700">
        <p>
          <strong>Estándar:</strong> Las características marcadas como estándar aparecerán pre-seleccionadas al crear nuevas parcelas.
          Puedes personalizar qué características tiene cada parcela (superficie, orientación, agua, césped, etc.) desde su ficha de edición.
        </p>
      </div>
    </div>
  );
