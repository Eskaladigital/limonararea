...
export async function middleware(request: NextRequest) {
  ...
  const blogCategoryMatch = pathname.match(/^\/(es|en|fr|de|nl)\/blog\/([^/]+)\/([^/]+)\/?$/);
  ...
  const localeMatch = pathname.match(/^\/(en|fr|de|nl)(\/.*)/);
  if (localeMatch) {
  ...
  if (!shouldSkip) {
    ...
    if (locale) {
      // ✅ NUEVA ARQUITECTURA: Carpetas fijas por idioma (/es/, /en/, /fr/, /de/, /nl/)
