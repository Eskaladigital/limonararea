 ...
 * Se envía el MISMO email tanto al cliente como a COMPANY_EMAIL (reservas)
 */
export async function sendBookingCreatedEmail(
  ...
  try {
    ...
    const subject = `Eco Area Limonar | Reserva pendiente - ${subjectInfo}`;
...
export async function sendFirstPaymentConfirmedEmail(
  ...
  try {
    ...
    const subjectInfo = getSubjectInfo(bookingData);
    const subject = `Eco Area Limonar | Reserva confirmada - ${subjectInfo}`;
...
export async function sendSecondPaymentConfirmedEmail(
  ...
  try {
    ...
    const subjectInfo = getSubjectInfo(bookingData);
    const subject = `Eco Area Limonar | Pago completo - ${subjectInfo}`;
