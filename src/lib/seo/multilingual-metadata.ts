 ...
 * @param path - Ruta base sin prefijo de idioma (ej: "/parcelas")
...
export function generateMultilingualMetadata(
  ...
  const currentMetadata = metadata[currentLang as keyof typeof metadata] || metadata['en'];
...
export function buildCanonicalAlternates(path: string, currentLang: Locale) {
  ...
  const pathWithoutLocale = cleanPath.replace(/^\/(es|en|fr|de|nl)/, '') || '/';
